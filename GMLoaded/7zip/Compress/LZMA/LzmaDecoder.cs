...
namespace SevenZip.Compression.LZMA
{
    public class Decoder : ICoder, ISetDecoderProperties // ,System.IO.Stream
    {
        ...
        private readonly LZ.OutWindow m_OutWindow = new LZ.OutWindow();
        ...
        private UInt32 m_DictionarySize;

        private UInt32 m_DictionarySizeCheck;

        private UInt32 m_PosStateMask;
        ...
        private void Init(System.IO.Stream inStream, System.IO.Stream outStream)
        {
            ...
            {
                for (UInt32 j = 0; j <= this.m_PosStateMask; j++)
                {
        ...
        private void SetDictionarySize(UInt32 dictionarySize)
        {
            if (this.m_DictionarySize != dictionarySize)
            {
                ...
                UInt32 blockSize = Math.Max(this.m_DictionarySizeCheck, 1 << 12);
                this.m_OutWindow.Create(blockSize);
            }
        }

        private void SetLiteralProperties(Int32 lp, Int32 lc)
        {
            if (lp > 8)
        ...
        private void SetPosBitsProperties(Int32 pb)
        {
            ...
            Int64 inSize, Int64 outSize, ICodeProgress progress)
        {
            ...
            while (nowPos64 < outSize64)
            {
                ...
                // while(nowPos64 < next)
                {
                    UInt32 posState = (UInt32)nowPos64 & this.m_PosStateMask;
                    if (this.m_IsMatchDecoders[(state.Index << Base.kNumPosStatesBitsMax) + posState].Decode(this.m_RangeDecoder) == 0)
                    {
                        ...
                        b = !state.IsCharState()
                            ? this.m_LiteralDecoder.DecodeWithMatchByte(this.m_RangeDecoder,
                                (UInt32)nowPos64, prevByte, this.m_OutWindow.GetByte(rep0))
                            : this.m_LiteralDecoder.DecodeNormal(this.m_RangeDecoder, (UInt32)nowPos64, prevByte);
                        this.m_OutWindow.PutByte(b);
                        state.UpdateChar();
                        nowPos64++;
                    }
                    else
                    {
                        ...
                        else
                        {
                            ...
                            state.UpdateMatch();
                            UInt32 posSlot = this.m_PosSlotDecoder[Base.GetLenToPosState(len)].Decode(this.m_RangeDecoder);
        ...
        private class LenDecoder
        {
            ...
            private UInt32 m_NumPosStates = 0;

            public void Create(UInt32 numPosStates)
            {
                for (UInt32 posState = this.m_NumPosStates; posState < numPosStates; posState++)
                {
            ...
            {
                this.m_Choice.Init();
                for (UInt32 posState = 0; posState < this.m_NumPosStates; posState++)
                {
        ...
        private class LiteralDecoder
        {
            ...
            private UInt32 GetState(UInt32 pos, Byte prevByte) => ((pos & this.m_PosMask) << this.m_NumPrevBits) + (UInt32)(prevByte >> (8 - this.m_NumPrevBits));
            ...
            public Byte DecodeNormal(RangeCoder.Decoder rangeDecoder, UInt32 pos, Byte prevByte) => this.m_Coders[this.GetState(pos, prevByte)].DecodeNormal(rangeDecoder);

            public Byte DecodeWithMatchByte(RangeCoder.Decoder rangeDecoder, UInt32 pos, Byte prevByte, Byte matchByte) => this.m_Coders[this.GetState(pos, prevByte)].DecodeWithMatchByte(rangeDecoder, matchByte);
            ...
            private struct Decoder2
            {
                ...
                public Byte DecodeWithMatchByte(RangeCoder.Decoder rangeDecoder, Byte matchByte)
                {
                    ...
                    do
                    {
                        ...
                        matchByte <<= 1;
                        UInt32 bit = this.m_Decoders[((1 + matchBit) << 8) + symbol].Decode(rangeDecoder);
                        symbol = (symbol << 1) | bit;
                        if (matchBit != bit)
                        {
                            while (symbol < 0x100)
                ...
                public void Init()
                {
                    for (Int32 i = 0; i < 0x300; i++) this.m_Decoders[i].Init();
                }
            }
        };
